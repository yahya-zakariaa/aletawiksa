    ...
    const loadProductData = async () => {
      if (params.category && modules[params.category]) {
        try {
          ...
          const categoryData = dataModule.default || dataModule;

          // إذا كان هناك نوع محدد في params
          if (params.type) {
            const productsData = categoryData[params.type];
            if (productsData) {
              setProducts(productsData);
              setFilteredProducts(productsData);
            } else {
              console.warn("No products found for type:", params.type);
              toast.error("لا توجد منتجات في هذا التصنيف");
            }
          } else {
            // إذا لم يكن هناك نوع محدد، نجمع كل المنتجات من التصنيف
            const allProducts = Object.values(categoryData).reduce(
                  ...
                  <Image
                    ...
                    className={`mx-auto w-full rounded-2xl ${
                      p.name.includes("17") || p.name.includes("Air")
                        ...
                        : "object-contain sm:scale-90 scale-80"
                    }`}
                  />
                  {p.disc > 0 && (
